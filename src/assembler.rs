...
pub fn ast_to_tam(ast: Expr) ->  Vec<TAMInst> {
    match ast {
        ...
        Expr::BinOp(operator, left, right) => ast_to_tam(*left).into_iter().chain(ast_to_tam(*right)).chain(std::iter::once(get_tamOP_from_astOP(operator))).collect(),
        Expr::UnOp(operator, value) => ast_to_tam(*value).into_iter().chain(std::iter::once(get_tamOP_from_astOP(operator))).collect(),
    }

}

fn get_tamOP_from_astOP(operator: TokenOperator) -> TAMInst {
    match operator {
